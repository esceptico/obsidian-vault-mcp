...
from headless_obsidian_mcp.core.config import EmbeddingSettings, VaultSettings
...
class VaultTests(unittest.TestCase):
    ...
    def test_create_read_search(self) -> None:
        ...
        with tmp:
            ...
            self.assertTrue(
                (Path(tmp.name) / ".headless-obsidian-mcp" / "index.sqlite").exists()
            )
            self.assertNotIn(
                ".headless-obsidian-mcp", {entry["path"] for entry in self._list(vault)}
            )

            with self.assertRaises(ValueError):
                self._create(vault, ".headless-obsidian-mcp/manual", "nope")
    ...
    def test_delete_refuses_headless_obsidian_mcp(self) -> None:
        tmp, vault = self.make_vault()
        with tmp:
    ...
    def test_trash_does_not_overwrite_same_second(self) -> None:
        ...
        with tmp:
            ...
            with patch("headless_obsidian_mcp.vault.service.datetime", FrozenDateTime):
    ...
    def test_create_note_rejects_oversized_content(self) -> None:
        from headless_obsidian_mcp.core.constants import MAX_NOTE_BYTES
    ...
    def test_create_note_rejects_oversized_rendered_note(self) -> None:
        ...
        tmp, vault = self.make_vault()
        with tmp, patch("headless_obsidian_mcp.vault.notes.MAX_NOTE_BYTES", 20):
            with self.assertRaises(ValueError):
    ...
    def test_update_note_rejects_oversized_rendered_note(self) -> None:
        ...
        tmp, vault = self.make_vault()
        with tmp, patch("headless_obsidian_mcp.vault.notes.MAX_NOTE_BYTES", 40):
            self._create(vault, "Note", "body")
            with self.assertRaises(ValueError):
                vault.update_note(
    ...
    def test_atomic_write_unique_tmp_and_fsync(self) -> None:
        ...
        with tmp:
            ...
            with patch(
                "headless_obsidian_mcp.vault.service.os.fsync", side_effect=spy_fsync
            ):
                self._create(vault, "Note", "hello")

            self.assertGreaterEqual(called.get("count", 0), 1)

            from headless_obsidian_mcp.vault.paths import temporary_write_path
