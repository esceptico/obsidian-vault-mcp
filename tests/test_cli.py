...
from headless_obsidian_mcp.app.cli import cli, run_server
...
class CliTests(unittest.TestCase):
    def test_run_port_zero_overrides_environment(self) -> None:
        with (
            ...
            patch("headless_obsidian_mcp.app.cli.configure_default_logging"),
            patch("headless_obsidian_mcp.app.cli.serve_main") as serve_main,
        ):
            run_server(None, 0)
            self.assertEqual(os.environ["HEADLESS_OBSIDIAN_MCP_PORT"], "0")
            serve_main.assert_called_once_with()

    def test_run_delegates_to_server(self) -> None:
        runner = CliRunner()
        with patch("headless_obsidian_mcp.app.cli.run_server") as run:
    ...
    def test_start_delegates_to_daemon(self) -> None:
        ...
        with patch(
            "headless_obsidian_mcp.app.daemon.DaemonService.from_settings",
